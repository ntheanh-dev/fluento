...
const Dashboard = () => {
  ...
  const hasPracticeHistory =
    (profile?.embedded?.totalUserSentenceAnswers ?? 0) > 0 ||
    ...
    () => [
      {
        ...
        value: creditBalance?.credits ?? 0,
        icon: Coins,
        color: "text-amber-600",
        bg: "bg-amber-50 dark:bg-amber-950/40",
      },
      {
        label: t("dashboard.totalAnswers"),
        value: profile?.embedded?.totalUserSentenceAnswers ?? 0,
        icon: FileText,
        color: "text-blue-600",
      ...
      },
      {
        ...
        value: (profile?.embedded?.avgUserSentenceAnswerScore ?? 0).toFixed(
          2,
        ),
        icon: CheckCircle2,
      ...
      },
      {
        ...
        value: formatTotalHours(profile?.embedded?.totalLearningTime ?? 0),
        icon: History,
        color: "text-purple-600",
        bg: "bg-purple-50 dark:bg-purple-950/40",
      },
    ],
    [t, creditBalance?.credits, profile?.embedded],
  );
    ...
    <div className="max-w-7xl mx-auto p-4 sm:px-6 lg:px-8 space-y-8 pb-8 dark:text-slate-100">
          ...
          {hasPracticeHistory ? (
            <>
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100">
                {t("dashboard.welcomeBack", { name: profile?.fullName ?? "" })}
              </h1>
              <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm md:text-base">
                {t("dashboard.streakLine", {
                  days: profile?.longestStreak ?? 0,
                })}
              </p>
            </>
          ) : (
            <>
              ...
              {profile?.activeApiKeyId ? (
                ...
                <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm md:text-base">
                  <Trans
                    i18nKey="dashboard.addApiKey"
                    components={{
                      profileLink: (
                        <Link
                          to="/profile"
                          className="text-[#198de6] font-semibold underline"
                        />
                      ),
                    }}
                  />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="bg-white dark:bg-slate-900/90 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4"
          >
            <div
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900/90 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            <select
              value={range}
              onChange={(e) =>
                setRange(e.target.value as WritingPerformanceRange)
              }
            ...
            >
              <option value="LAST_7_DAYS">{t("dashboard.range7")}</option>
              <option value="LAST_30_DAYS">{t("dashboard.range30")}</option>
            </select>
          </div>
