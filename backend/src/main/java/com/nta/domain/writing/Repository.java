...
@org.springframework.stereotype.Repository("writingRepository")
public interface Repository extends JpaRepository<Writing, Long> {
    ...
    @Query(
            "SELECT w FROM Writing w WHERE w.topic IS NOT NULL AND LOWER(CAST(w.topic AS string)) LIKE LOWER(CONCAT('%', :keyword, '%'))")
    ...
    @Query(
            "SELECT w FROM Writing w WHERE w.user.id = :userId AND "
                    ...
                    + "OR w.type = com.nta.domain.writing.WritingType.CUSTOM_TEXT AND LOWER('Custom Text') LIKE LOWER(CONCAT('%', :keyword, '%')))")
